...
function mainContent()
{
    ...
    if (!isset($_GET['course']) || $_GET['course'] === 'view' || $user) {
        ...
        if (isset($_GET['course'])) {
            ...
            if ($_GET['course'] == 'view') {
                $theme = new themer('training/course_info');
                $account = '';
                ...
                if ($_GET['process'] == 'payment') {
                    $theme = new themer('training/course_get');
                    $account = '';
                    // Set the page title
                    ...
                    $theme = new themer('training/payment_status');
                    $account = '';
                    // Set the page title
                    ...
                    if (!$check && isset($_GET['type']) && $_GET['type'] == 'canceled') {
                        ...
                        if ($empty_error) {
                            $empty = '<div class="empty">
                                ...
                                <p class="small para">Payment Error</p>
                                </div>';
                        }
                        $PTMPL['empty'] = $empty;
                    ...
                    if ($check == 1 || $check == 2) {
                        if ($check == 2) {
                            $PTMPL['success'] = messageNotice('Your payment has completed successfully, thanks for your interest in learning', 1);
                            $PTMPL['learning'] = simpleButtons(null, 'Start Learning', isset($_GET['referrer']) ? $_GET['referrer'] : '');
                        } else {
                            $PTMPL['success'] = messageNotice('You already purchased this course, you can continue learning or try purchasing another course!');
                ...
                $theme = new themer('training/course_add_edit');
                $account = '';
                ...
                $theme = new themer('training/link_module');
                $account = '';
                // $OLD_THEME = $PTMPL; $PTMPL = array();
                ...
                if ($grant_access) {
                    $theme = new themer('training/now_learning');
                    $account = '';
                    // $OLD_THEME = $PTMPL; $PTMPL = array();
                    //
        ...
        <div class="m-2">';
            ...
            if ($_GET['module'] == 'add' || $_GET['module'] == 'edit') {
                $theme = new themer('training/module_add_edit');
                $account = '';
                // $OLD_THEME = $PTMPL; $PTMPL = array();
            ...
            $theme = new themer('training/home');
            $account = '';
            // $OLD_THEME = $PTMPL; $PTMPL = array();
